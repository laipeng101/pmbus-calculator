...
- **位映射面板展开状态记忆**：16 位 Raw Word 与 VOUT_MODE 8 位面板均默认
  展开，并提供明确标题及键盘/触摸可操作的收起入口。两项偏好独立持久化，
  切换模式、刷新后恢复；16 位偏好由四种数值模式共享，8 位偏好由独立
  VOUT_MODE 与 L16 内嵌配置器共享。存储不可用或数据损坏时安全回退。
- **结果上下文**：结果旁展示 raw、格式和参数来源；L16 显式区分标称参考
  缺失与零，DIRECT 展示当前系数并提示核对器件数据手册。

### Fixed

- **位映射可发现性**：16 位网格移回 Raw Word 下方、参数面板之前，位值、
  nibble Hex 与图例直接可见。收起只改变显示偏好，Hex、参数和计算继续
  可用，展开后显示当前 raw；隐藏位按钮退出 Tab 顺序。
- **L11 等误差边界选择**：自动指数搜索纳入各指数下的合法尾数边界，
  等误差时遵循既定的较小指数绝对值规则；`1023.5` 编为 `03FF`、
  `-1025` 编为 `0400`（均 N=0），正确报告量化误差。
- **L11 Auto-N 请求来源**：锁定/解锁指数保留真实物理值请求，不把解码值
  冒充用户请求；只有 raw 输入时不凭空产生量化误差，显式零请求仍保留。
- **L16 物理值复制**：非 LINEAR 或缺少标称参考等结果不可用状态禁用
  物理值复制并显示原因，不再复制占位符；真实零结果仍可复制。
- **界面可读性和输入布局**：提高亮暗主题普通文本在实际面板背景上的
  对比度；保留结果优先布局与首屏 Raw/位映射/物理值；DIRECT 系数输入
  对齐并关联可访问标签，错误信息不再使各输入错位。

### Changed

- Firefox/WebKit 核心 smoke 使用真实编码、解码变化向量，并覆盖位映射
  收起/刷新/展开；增加移动端真实触摸、键盘与存储回退回归。
- 部署文档对齐 immutable Release、annotated tag 与允许回滚范围；PR
  验收统计明确采用最终 committed base→head 与 checked tree。
- 视觉基线逐图审查后更新，并增加桌面 16 位/移动 8 位收起场景。

## [3.1.1] - 2026-09-06

### Fixed

- **DIRECT 物理值复制回录静默改变 raw（P1，显示文本回录合同统一）**：
  v2.5.11 的保真谓词是 binary64 管线回程，对**显示格式化折叠**全盲——
  m=1、b=1、R=12、raw FFFF 显示 `-1`（无任何告警），但显示文本经真实
  typed 提交（classify → 精确 parse → 精确 encode）编码为 Y=0：该系数
  全 65536 个 Y 中 29491 个的「物理值」复制回录会静默改变 raw；(7,3,12)
  等族 56172/65536 不安全且 0 告警；(1,1,17) 族也有 4427 个旧守卫安静但
  复制不安全的状态。修复：保真谓词统一为「显示文本 → 真实 typed 编码
  回程」（新单一事实源 `analyzeDirectTextReentry`，ADR 0007），全部表面
  （警告、量化注记、计算步骤精确值行、复制 override/disable、值文本）
  共享同一解析；经验证回录文本生成器以 `exact`/`approximate` kind 如实
  标注文本性质（循环有理数的经验证近似不再冒充精确有限小数）；m=0 时
  物理值复制禁用并给出可访问原因（不再复制 `—` 占位符）。受保护算法、
  typed 提交合同、4096 资源门禁、provenance、untouched blur、复制格式
  与持久化零变更；显示文本 policy（ADR 0005）不变。
- 新增全 65536-Y「最终复制文本 → 真实 typed 编码 → raw 不变」性质测试
  （(1,1,12) 全族 + 既有 sweep corpora）、损失分层（binary64 表示损失 vs
  显示格式化损失）golden 与真实键盘/剪贴板回录 E2E（两个失败族 + 安静
  对照 + m=0 禁用）；DOMAIN_MODEL §2.3、UI_CONVENTIONS §15、ADR 0007 同步。
