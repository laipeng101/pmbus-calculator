...
| 调试测试        | `runBoundaryTests`                   | Vitest + `DebugDrawer`                       | 部分完成 | Vitest 已接入 77 pass（13 math + 38 reducer + 26 view-model）；`DebugDrawer` 显示“51/51”过时；边界测试入口待 M8 接入 |
...
| M3 L11    | 已完成 | 已完成 | 已完成 | 已完成 | 已完成 | 已完成   | 部分（decode 半闭环） | 不适用                | 不适用                | 不适用                |
| M4 L16    | 已完成 | 已完成 | 已完成 | 已完成 | 已完成 | 已完成   | 部分（decode 半闭环） | 部分（decode 半闭环） | 不适用                | 不适用                |
| M5 DIRECT | 已完成 | 已完成 | 已完成 | 已完成 | 已完成 | 已完成   | 部分（decode 半闭环） | 部分（decode 半闭环） | 部分（decode 半闭环） | 不适用                |
| M6 HALF   | 已完成 | 已完成 | 已完成 | 已完成 | 已完成 | 已完成   | 部分（decode 半闭环） | 部分（decode 半闭环） | 部分（decode 半闭环） | 部分（decode 半闭环） |
