...
- L11 自动 N 切换保留已有显式请求；没有请求时，可按当前解码值重新选择编码，但
  `l11.valueInput` 仍为 `null`，不得把解码值当成用户请求或生成零误差读数。
...
- 绝对误差格式化必须保证**任何非零差值绝不渲染为文本零**：`|x| ≥ 1e-6` 使用
  ...
  其自适应分支与全部表面的通用 plain-text 数值呈现共用同一 canonical policy
  （`src/app/numeric-presentation.ts`，见 ADR 0005）。
