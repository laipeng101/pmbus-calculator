...
- 纯计算器不需要命令下拉框：命令选择不能可靠推导数据格式（器件数据手册或
  `VOUT_MODE` 决定），因此主工作区移除 `CommandPicker`，改为默认折叠的只读
  “命令参考”面板。
- 命令参考完全无副作用：不选择、不切换模式、不注入参数、不重编码 raw。
- `command/apply-preset` action、`CommandPreset` 类型与所有 preset 元数据已
  从产品面移除；演示值不再作为任何标准定义的附属。
- 本 ADR 的“两层分离”结论仍有效：标准定义与演示/设备数据分离的原则不变，
  只是演示层不再通过 UI 暴露。
