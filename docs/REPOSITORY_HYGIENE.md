...
9. 不为了减小仓库而使用有损压缩。
10. 视觉重构 PR（重构组件/CSS、统一类名、调整布局等不改变交互行为的变更）更新基线时，
    必须附**新旧基线 diff 审查记录**：
    - 更新前先运行 `npm run test:e2e:visual`，把失败场景清单记为预期影响面；
    - `--update-snapshots` 后对新旧 PNG 逐图（或按影响面成对）对比，确认每张变化与改动意图一致；
    - 核对实际更新集合与预期影响面吻合；预期受影响却未更新的基线必须解释原因；
    - 禁止"以新渲染结果直接为新基线"而不做新旧对比：基线锁定的是正确形态，不是当前渲染。
