...
- 首屏先呈现结果及其 raw / 格式 / 参数来源。16 位位映射紧邻 Raw Word 输入，
  默认展开，位值、nibble 和字段图例直接可见，参数输入位于其后；DOM 与键盘顺序一致。
  收起时保留明确的位映射标题与展开入口。页头不重复一整排格式术语入口，
  各模式标题、配置摘要及说明保留对应术语帮助。
- 桌面 1280×900 的默认页面总高度上限为 1500px（包含 L16 两条位映射标题）；
  主动收起位映射后仍满足 1400px 上限。分别验收这两个状态，并保留结果、Raw、
  默认 16 位网格与物理值的首屏几何门禁，不以收起状态替代默认可见性验收。
...
- 16 位与 VOUT_MODE 8 位映射均默认展开，可通过具备 `aria-expanded` / `aria-controls`
  的按钮收起；隐藏位按钮不参与 Tab 顺序。两种面板分别记忆展开状态，16 位状态在
  四种数值模式间共享，8 位状态在独立 VOUT_MODE 与 L16 内嵌配置器间共享。
  持久化只经过 `src/app/persistence.ts`；旧用户、缺失/损坏偏好或存储受限时安全回到
  默认展开。收起不改变 raw、配置、输入请求或结果，Hex 与语义配置保持可操作。
...
- **DIRECT exact 呈现（v2.5.12）**：DIRECT 面板数值由精确有理数渲染
  （`formatExactDelta` / `formatExactPercent`，与计算步骤同一来源）——整数差值
  显示为整数（`+1`），极小/极大值用带符号科学计数法（`+1e-16`、`-1e-19`），带内
  终止小数精确展开，循环有理数回退到精确分数（`-1/6`）并在注记/步骤中给出
  「约」十进制近似；相对百分比带内用 4 位小数、带外用科学计数法（`约 1e-15%`）。
  `deltaNote` 在显示值无法完整呈现请求时保留「用户请求 <lexeme>；raw 精确表示
  <exact>」（长文本 `break-all` 换行，不产生横向溢出）；请求文本与显示值一致时
  保持静默（安全普通向量不出现噪音注记）。饱和态显示 `<delta>（已编码到边界值）`。
- **DIRECT 精确 lexeme 长度边界（v2.5.12）**：`DIRECT_EXACT_MAX_LEXEME_LENGTH =
4096`（证据与语义见 DOMAIN_MODEL §2.3，是交互资源边界而非 PMBus 限制）。
  ValueInput 在 DIRECT 模式把超长草稿显示为明确的「输入过长，未提交」错误并保留
  旧 raw / 旧请求；**禁止用 `maxLength` 属性实现**——浏览器对超长粘贴的静默截断
  会把截断后的值当作新请求提交，违反「不静默截断」合同。分类错误（下溢/越界）
  的既有文案与优先级不受影响；超长检查在分类之前做 O(1) 拒绝。
...
- **DIRECT 回录保真表面（v2.5.11；v3.1.1 统一为显示文本合同）**：当**显示
  文本**经真实 typed 编码（classify → 精确 parse → 精确 encode，与 reducer
  同一合同）回编不是当前 Y 时（如 m=1、b=1、R=17 的 raw FFFF：精确
  -1.00000000000000001、显示 -1；或 m=1、b=1、R=12 的 raw FFFF：binary64
  值可安全回录但显示文本 "-1" 编码为 Y=0），view-model 的 `directFidelity`
  驱动三个表面——InfoPanel 警告 `direct-precision-fold`（明确显示值为近似、
  直接回输会编码为 Y=<displayReencodedY>，是"不同的请求"）、量化读数注记
  （`deltaKind` 降为 warn，不因 binary64 零误差读数读成干净 exact）与计算
  步骤的精确有理数/十进制行（循环小数显式声明无有限精确十进制）；「物理值」
  复制按钮经 `physicalValueCopyOverride` 返回经独立 exact encoder 验证、
  可安全回录的文本（附说明行，aria-describedby 指向该说明；无法生成验证
  文本时禁用复制并给出原因，绝不交出未验证字符串）。文案必须按 `lossKind`
  区分 binary64 表示损失与显示格式化损失，不得把所有折叠都称为「超出
  binary64 精度」；复制说明按 `safeReentryKind` 如实区分精确文本与循环
  小数的经验证近似文本。m=0 时物理值复制禁用并给出可访问原因（`—` 占位符
  不可回录）。安静状态不出现任何保真表面；raw/系数变化实时重算，untouched
  blur 严格 no-op 不变。警告文案不得把折叠状态称为 exact，也不得声称
  "浮点有误差可以忽略"。
...
- DIRECT 字节（`0x40`/`0xC0` 等）：状态与警告必须继续要求器件 m/b/R 系数
  （§7.4），relative 组合再叠加 VOUT_COMMAND 标称参考值与相对值必须为正（§8.5.2）；
  `0xC0` 的
...
- IEEE Half 字节（`0x60`/`0xE0` 等）：所有用户可见表面必须表述「标准
  ...
  **禁止出现「需器件资料」「器件 Profile」「m/b/R」「DIRECT 系数」等反向禁词**。
  `0xE0` 额外表述需要标称参考值与相对值必须为正（§8.5.2），`0x60` 不出现该表述。
