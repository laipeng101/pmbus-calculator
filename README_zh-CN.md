...
- 🎛️ **同一台仪表，五个档位** — 五种模式共享同一结果工作区：物理主值锚定左侧，右侧固定三行（字段/参数 → 通用关系式 → 当前数值代入），下方是四个稳定上下文槽位（Raw / 格式 / 参数 / 上下文）与仅由已提交输入来源推导的解码/编码方向。数值模式经 KaTeX 排版真实 LaTeX（HTML + MathML）；VOUT_MODE 保持同一节奏，渲染位域配置解析，绝不送入 KaTeX。
