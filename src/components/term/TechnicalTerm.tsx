...
export default function TechnicalTerm({ termId, children }: Props) {
  ...
  const panelId = useId()
  const { isActive, toggleTerm, registerSurface, closeIfActive } = useHelpOverlay()
  ...
  // If this term unmounts while its surface is the active one (mode switch,
  // conditional render), drop the surface — no stale trigger for the Escape
  // restore to aim at. Symmetric with ControlTooltip's unmount cleanup;
  ...
  return (
    <>
      <button
        ...
        onClick={(event) => toggleTerm(surfaceKey, event.currentTarget)}
      >
        {children ?? term.token}
      </button>
      {open &&
